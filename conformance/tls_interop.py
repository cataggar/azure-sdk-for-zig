#!/usr/bin/env python3
"""Run independent OpenSSL TLS servers with disposable, explicitly pinned test identities."""
...
def run_fixture(command, directory, algorithm, validity):
    ...
    try:
        ...
        subprocess.run([*command, str(der.resolve()), *(str(value) for value in ports), validity], check=True, timeout=180)
    finally:
...
def main():
    ...
    for algorithm in ("p256", "p384", "rsa"):
        run_fixture(sys.argv[1:], directory, algorithm, "valid")
    for validity in ("expired", "future"):
        run_fixture(sys.argv[1:], directory, "rsa", validity)
