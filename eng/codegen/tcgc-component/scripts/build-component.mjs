...
const jsImportEntries = []; // { virtualPath, packageSpec }
...
for (const spec of packages) {
  ...
  // Main JS module — registered in jsImports map, NOT inlined as text
  // (esbuild bundles the module body via the import in the entry file).
...
for (let i = 0; i < packages.length; i++) {
  entry += `import * as __pkg${i} from ${JSON.stringify(packages[i])};\n`;
}
entry += `\n`;
...
for (const e of jsImportEntries) {
  const pkgIdx = packages.indexOf(e.packageSpec);
...
await esbuild.build({
  ...
  target: "es2022",
  outfile: bundlePath,
  // Don't externalise node:* — bundle our own stubs in their place.
