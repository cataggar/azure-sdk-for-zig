//! An `HttpTransport` that performs outbound requests through the WASI
//! component-model `wasi:http/outgoing-handler@0.2.6` interface, so the
//! Azure SDK can run inside a WebAssembly component (wamr / wasmtime)
