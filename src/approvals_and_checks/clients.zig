...
pub const ApprovalsAndChecksClient = struct {
    ...
    pub fn checkConfigurations(self: *@This()) CheckConfigurations {
        return .{
            .endpoint = self.endpoint,
            .api_version = self.api_version,
            .pipeline = self.pipeline,
    ...
    pub fn checkEvaluations(self: *@This()) CheckEvaluations {
        return .{
            .endpoint = self.endpoint,
            .api_version = self.api_version,
            .pipeline = self.pipeline,
...
pub const PipelinePermissions = struct {
    ...
    pub fn updatePipelinePermisionsForResources(self: *@This(), alloc: std.mem.Allocator, organization: []const u8, project: []const u8, body: []const models.ResourcePipelinePermissions) !models.ResourcePipelinePermissionsList {
        ...
        const base_url = try std.fmt.allocPrint(alloc, "{s}/{s}/{s}/_apis/pipelines/pipelinepermissions", .{ self.endpoint, encoded_path_0, encoded_path_1 });
    ...
    pub fn get(self: *@This(), alloc: std.mem.Allocator, organization: []const u8, project: []const u8, resource_type: []const u8, resource_id: []const u8) !models.ResourcePipelinePermissions {
        ...
        const encoded_path_1 = try core.url.encodePathSegment(alloc, project);
        defer alloc.free(encoded_path_1);
        const encoded_path_2 = try core.url.encodePathSegment(alloc, resource_type);
        ...
        const base_url = try std.fmt.allocPrint(alloc, "{s}/{s}/{s}/_apis/pipelines/pipelinepermissions/{s}/{s}", .{ self.endpoint, encoded_path_0, encoded_path_1, encoded_path_2, encoded_path_3 });
        defer alloc.free(base_url);
        var url_buf: std.ArrayList(u8) = .empty;
        defer url_buf.deinit(alloc);
        try url_buf.appendSlice(alloc, base_url);
        ...
        if (!responseStatusExpected(resp.status_code, &.{200})) {
            core.pager.logHttpError("PipelinePermissions.get", resp.status_code, resp.body);
            return error.AzureRequestFailed;
        }
        return try serde.json.fromSlice(models.ResourcePipelinePermissions, alloc, resp.body);
    }
    ...
    pub fn updatePipelinePermisionsForResource(self: *@This(), alloc: std.mem.Allocator, organization: []const u8, project: []const u8, resource_type: []const u8, resource_id: []const u8, body: models.ResourcePipelinePermissions) !models.ResourcePipelinePermissions {
        ...
        if (!responseStatusExpected(resp.status_code, &.{200})) {
            core.pager.logHttpError("PipelinePermissions.updatePipelinePermisionsForResource", resp.status_code, resp.body);
            return error.AzureRequestFailed;
        }
        return try serde.json.fromSlice(models.ResourcePipelinePermissions, alloc, resp.body);
    }
...
pub const CheckConfigurations = struct {
    ...
    pub fn list(self: *@This(), alloc: std.mem.Allocator, organization: []const u8, project: []const u8, resource_type: ?[]const u8, resource_id: ?[]const u8, @"$expand": ?enums.ListRequestExpand) !models.CheckConfigurationList {
        ...
        const encoded_path_1 = try core.url.encodePathSegment(alloc, project);
        defer alloc.free(encoded_path_1);
        const base_url = try std.fmt.allocPrint(alloc, "{s}/{s}/{s}/_apis/pipelines/checks/configurations", .{ self.endpoint, encoded_path_0, encoded_path_1 });
        defer alloc.free(base_url);
        var url_buf: std.ArrayList(u8) = .empty;
        defer url_buf.deinit(alloc);
        try url_buf.appendSlice(alloc, base_url);
        var has_query = std.mem.indexOfScalar(u8, base_url, '?') != null;
        if (resource_type) |query_value| {
            const sep: []const u8 = if (has_query) "&" else "?";
            const enc = try core.url.percentEncode(alloc, query_value);
            defer alloc.free(enc);
            try url_buf.print(alloc, "{s}resourceType={s}", .{ sep, enc });
            has_query = true;
        }
        if (resource_id) |query_value| {
            const sep: []const u8 = if (has_query) "&" else "?";
            const enc = try core.url.percentEncode(alloc, query_value);
            defer alloc.free(enc);
            try url_buf.print(alloc, "{s}resourceId={s}", .{ sep, enc });
            has_query = true;
        }
    ...
    pub fn add(self: *@This(), alloc: std.mem.Allocator, organization: []const u8, project: []const u8, body: models.CheckConfiguration) !models.CheckConfiguration {
        ...
        if (!responseStatusExpected(resp.status_code, &.{200})) {
            core.pager.logHttpError("CheckConfigurations.add", resp.status_code, resp.body);
            return error.AzureRequestFailed;
        }
    ...
    pub fn delete(self: *@This(), alloc: std.mem.Allocator, organization: []const u8, project: []const u8, id: i32) !void {
        ...
        defer alloc.free(encoded_path_2);
        const base_url = try std.fmt.allocPrint(alloc, "{s}/{s}/{s}/_apis/pipelines/checks/configurations/{s}", .{ self.endpoint, encoded_path_0, encoded_path_1, encoded_path_2 });
        defer alloc.free(base_url);
        var url_buf: std.ArrayList(u8) = .empty;
        defer url_buf.deinit(alloc);
        try url_buf.appendSlice(alloc, base_url);
        ...
        if (!responseStatusExpected(resp.status_code, &.{200})) {
            core.pager.logHttpError("CheckConfigurations.delete", resp.status_code, resp.body);
            return error.AzureRequestFailed;
        }
        return;
    }
    ...
    pub fn get(self: *@This(), alloc: std.mem.Allocator, organization: []const u8, project: []const u8, id: i32, @"$expand": ?enums.GetRequestExpand) !models.CheckConfiguration {
        ...
        if (!responseStatusExpected(resp.status_code, &.{200})) {
            core.pager.logHttpError("CheckConfigurations.get", resp.status_code, resp.body);
            return error.AzureRequestFailed;
        }
        return try serde.json.fromSlice(models.CheckConfiguration, alloc, resp.body);
    }
    ...
    pub fn update(self: *@This(), alloc: std.mem.Allocator, organization: []const u8, project: []const u8, id: i32, body: models.CheckConfiguration) !models.CheckConfiguration {
        ...
        if (!responseStatusExpected(resp.status_code, &.{200})) {
            core.pager.logHttpError("CheckConfigurations.update", resp.status_code, resp.body);
            return error.AzureRequestFailed;
        }
        return try serde.json.fromSlice(models.CheckConfiguration, alloc, resp.body);
    }
    ...
    pub fn query(self: *@This(), alloc: std.mem.Allocator, organization: []const u8, project: []const u8, @"$expand": ?enums.QueryRequestExpand, body: []const models.Resource) !models.CheckConfigurationList {
        ...
        const encoded_path_1 = try core.url.encodePathSegment(alloc, project);
        defer alloc.free(encoded_path_1);
        const base_url = try std.fmt.allocPrint(alloc, "{s}/{s}/{s}/_apis/pipelines/checks/queryconfigurations", .{ self.endpoint, encoded_path_0, encoded_path_1 });
        defer alloc.free(base_url);
        var url_buf: std.ArrayList(u8) = .empty;
        defer url_buf.deinit(alloc);
        try url_buf.appendSlice(alloc, base_url);
...
pub const CheckEvaluations = struct {
    ...
    pub fn evaluate(self: *@This(), alloc: std.mem.Allocator, organization: []const u8, project: []const u8, @"$expand": ?enums.EvaluateRequestExpand, body: models.CheckSuiteRequest) !models.CheckSuite {
        ...
        const encoded_path_1 = try core.url.encodePathSegment(alloc, project);
        defer alloc.free(encoded_path_1);
        const base_url = try std.fmt.allocPrint(alloc, "{s}/{s}/{s}/_apis/pipelines/checks/runs", .{ self.endpoint, encoded_path_0, encoded_path_1 });
        defer alloc.free(base_url);
        var url_buf: std.ArrayList(u8) = .empty;
        defer url_buf.deinit(alloc);
        try url_buf.appendSlice(alloc, base_url);
        ...
        if (!responseStatusExpected(resp.status_code, &.{200})) {
            core.pager.logHttpError("CheckEvaluations.evaluate", resp.status_code, resp.body);
            return error.AzureRequestFailed;
        }
        return try serde.json.fromSlice(models.CheckSuite, alloc, resp.body);
    }
    ...
    pub fn get(self: *@This(), alloc: std.mem.Allocator, organization: []const u8, project: []const u8, check_suite_id: []const u8, @"$expand": ?enums.GetRequestExpand1) !models.CheckSuite {
        ...
        const encoded_path_1 = try core.url.encodePathSegment(alloc, project);
        defer alloc.free(encoded_path_1);
        const encoded_path_2 = try core.url.encodePathSegment(alloc, check_suite_id);
        defer alloc.free(encoded_path_2);
        const base_url = try std.fmt.allocPrint(alloc, "{s}/{s}/{s}/_apis/pipelines/checks/runs/{s}", .{ self.endpoint, encoded_path_0, encoded_path_1, encoded_path_2 });
        defer alloc.free(base_url);
        var url_buf: std.ArrayList(u8) = .empty;
        defer url_buf.deinit(alloc);
        try url_buf.appendSlice(alloc, base_url);
        ...
        if (!responseStatusExpected(resp.status_code, &.{200})) {
            core.pager.logHttpError("CheckEvaluations.get", resp.status_code, resp.body);
            return error.AzureRequestFailed;
        }
        return try serde.json.fromSlice(models.CheckSuite, alloc, resp.body);
    }
    ...
    pub fn update(self: *@This(), alloc: std.mem.Allocator, organization: []const u8, project: []const u8, check_suite_id: []const u8, @"$expand": ?enums.UpdateRequestExpand, body: models.CheckSuiteUpdateParameter) !models.CheckSuite {
        ...
        if (!responseStatusExpected(resp.status_code, &.{200})) {
            core.pager.logHttpError("CheckEvaluations.update", resp.status_code, resp.body);
            return error.AzureRequestFailed;
        }
        return try serde.json.fromSlice(models.CheckSuite, alloc, resp.body);
    }
...
pub const Approvals = struct {
    ...
    pub fn query(self: *@This(), alloc: std.mem.Allocator, organization: []const u8, project: []const u8, approval_ids: ?[]const u8, @"$expand": ?enums.QueryRequestExpand, assigned_to: ?[]const u8, state: ?enums.QueryRequestState, top: ?i32) !models.ApprovalList {
        ...
        const encoded_path_1 = try core.url.encodePathSegment(alloc, project);
        defer alloc.free(encoded_path_1);
        const base_url = try std.fmt.allocPrint(alloc, "{s}/{s}/{s}/_apis/pipelines/approvals", .{ self.endpoint, encoded_path_0, encoded_path_1 });
        defer alloc.free(base_url);
        var url_buf: std.ArrayList(u8) = .empty;
        defer url_buf.deinit(alloc);
        try url_buf.appendSlice(alloc, base_url);
        var has_query = std.mem.indexOfScalar(u8, base_url, '?') != null;
        if (approval_ids) |query_value| {
            const sep: []const u8 = if (has_query) "&" else "?";
            const enc = try core.url.percentEncode(alloc, query_value);
            defer alloc.free(enc);
            try url_buf.print(alloc, "{s}approvalIds={s}", .{ sep, enc });
            has_query = true;
        }
        if (@"$expand") |query_value| {
            const sep: []const u8 = if (has_query) "&" else "?";
        ...
        if (assigned_to) |query_value| {
            const sep: []const u8 = if (has_query) "&" else "?";
            const enc = try core.url.percentEncode(alloc, query_value);
            defer alloc.free(enc);
            try url_buf.print(alloc, "{s}assignedTo={s}", .{ sep, enc });
            has_query = true;
        }
    ...
    pub fn get(self: *@This(), alloc: std.mem.Allocator, organization: []const u8, project: []const u8, approval_id: []const u8, @"$expand": ?enums.GetRequestExpand) !models.Approval {
        ...
        defer alloc.free(encoded_path_2);
        const base_url = try std.fmt.allocPrint(alloc, "{s}/{s}/{s}/_apis/pipelines/approvals/{s}", .{ self.endpoint, encoded_path_0, encoded_path_1, encoded_path_2 });
        defer alloc.free(base_url);
        var url_buf: std.ArrayList(u8) = .empty;
        defer url_buf.deinit(alloc);
        try url_buf.appendSlice(alloc, base_url);
        ...
        if (!responseStatusExpected(resp.status_code, &.{200})) {
            core.pager.logHttpError("Approvals.get", resp.status_code, resp.body);
            return error.AzureRequestFailed;
        }
        return try serde.json.fromSlice(models.Approval, alloc, resp.body);
    }
};
