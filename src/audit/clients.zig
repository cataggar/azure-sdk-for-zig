...
const default_endpoint = "https://auditservice.dev.azure.com";
...
pub const AuditClient = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, options: InitOptions) !AuditClient {
    ...
    pub fn actions(self: *@This()) Actions {
        return .{
    ...
    pub fn auditLog(self: *@This()) AuditLog {
        return .{
            .endpoint = self.endpoint,
            .api_version = self.api_version,
            .pipeline = self.pipeline,
    ...
    pub fn downloadLog(self: *@This()) DownloadLog {
        return .{
            .endpoint = self.endpoint,
            .api_version = self.api_version,
            .pipeline = self.pipeline,
    ...
    pub fn streams(self: *@This()) Streams {
        return .{
            .endpoint = self.endpoint,
            .api_version = self.api_version,
            .pipeline = self.pipeline,
...
pub const AuditLog = struct {
    ...
    pub fn query(self: *@This(), alloc: std.mem.Allocator, organization: []const u8, start_time: ?[]const u8, end_time: ?[]const u8, batch_size: ?i32, continuation_token: ?[]const u8, skip_aggregation: ?bool) !models.AuditLogQueryResult {
        ...
        if (skip_aggregation) |query_value| {
            const sep: []const u8 = if (has_query) "&" else "?";
            try url_buf.print(alloc, "{s}skipAggregation={}", .{ sep, query_value });
            has_query = true;
        }
        ...
        if (!responseStatusExpected(resp.status_code, &.{200})) {
            core.pager.logHttpError("AuditLog.query", resp.status_code, resp.body);
            return error.AzureRequestFailed;
        }
        return try serde.json.fromSlice(models.AuditLogQueryResult, alloc, resp.body);
    }
};

pub const DownloadLog = struct {
    endpoint: []const u8,
    api_version: []const u8,
    pipeline: core.pipeline.HttpPipeline,

    pub const DownloadLogResult = union(enum) {
        status_200: struct {
            status: u16 = 200,
            headers: struct {
    ...
    pub fn downloadLog(self: *@This(), alloc: std.mem.Allocator, organization: []const u8, format: []const u8, start_time: ?[]const u8, end_time: ?[]const u8) !DownloadLogResult {
        ...
        try url_buf.appendSlice(alloc, base_url);
        var has_query = std.mem.indexOfScalar(u8, base_url, '?') != null;
        const encoded_query_0 = try core.url.percentEncode(alloc, format);
        defer alloc.free(encoded_query_0);
        try url_buf.print(alloc, "{s}format={s}", .{ if (has_query) "&" else "?", encoded_query_0 });
        has_query = true;
        if (start_time) |query_value| {
            const sep: []const u8 = if (has_query) "&" else "?";
            const enc = try core.url.percentEncode(alloc, query_value);
        ...
        switch (resp.status_code) {
            200 => {
                ...
                core.pager.logHttpError("DownloadLog.downloadLog", resp.status_code, resp.body);
                return error.AzureRequestFailed;
            },
        }
    }
...
pub const Streams = struct {
    ...
    pub fn create(self: *@This(), alloc: std.mem.Allocator, organization: []const u8, days_to_backfill: i32, body: models.AuditStream) !models.AuditStream {
        ...
        try url_buf.appendSlice(alloc, base_url);
        var has_query = std.mem.indexOfScalar(u8, base_url, '?') != null;
        try url_buf.print(alloc, "{s}daysToBackfill={d}", .{ if (has_query) "&" else "?", days_to_backfill });
        has_query = true;
        const encoded_query_1 = try core.url.percentEncode(alloc, self.api_version);
        defer alloc.free(encoded_query_1);
        try url_buf.print(alloc, "{s}api-version={s}", .{ if (has_query) "&" else "?", encoded_query_1 });
        ...
        if (!responseStatusExpected(resp.status_code, &.{200})) {
            core.pager.logHttpError("Streams.create", resp.status_code, resp.body);
            return error.AzureRequestFailed;
        }
        return try serde.json.fromSlice(models.AuditStream, alloc, resp.body);
    }
    ...
    pub fn updateStream(self: *@This(), alloc: std.mem.Allocator, organization: []const u8, body: models.AuditStream) !models.AuditStream {
        ...
        if (!responseStatusExpected(resp.status_code, &.{200})) {
            core.pager.logHttpError("Streams.updateStream", resp.status_code, resp.body);
            return error.AzureRequestFailed;
        }
        return try serde.json.fromSlice(models.AuditStream, alloc, resp.body);
    }
    ...
    pub fn delete(self: *@This(), alloc: std.mem.Allocator, organization: []const u8, stream_id: i32) !void {
        ...
        const base_url = try std.fmt.allocPrint(alloc, "{s}/{s}/_apis/audit/streams/{s}", .{ self.endpoint, encoded_path_0, encoded_path_1 });
        defer alloc.free(base_url);
        var url_buf: std.ArrayList(u8) = .empty;
        defer url_buf.deinit(alloc);
        try url_buf.appendSlice(alloc, base_url);
        ...
        if (!responseStatusExpected(resp.status_code, &.{200})) {
            core.pager.logHttpError("Streams.delete", resp.status_code, resp.body);
            return error.AzureRequestFailed;
        }
        return;
    }
    ...
    pub fn queryStreamById(self: *@This(), alloc: std.mem.Allocator, organization: []const u8, stream_id: i32) !models.AuditStream {
        ...
        if (!responseStatusExpected(resp.status_code, &.{200})) {
            core.pager.logHttpError("Streams.queryStreamById", resp.status_code, resp.body);
            return error.AzureRequestFailed;
        }
        return try serde.json.fromSlice(models.AuditStream, alloc, resp.body);
    }
    ...
    pub fn updateStatus(self: *@This(), alloc: std.mem.Allocator, organization: []const u8, stream_id: i32, status: enums.UpdateStatusRequestStatus) !models.AuditStream {
        ...
        try url_buf.appendSlice(alloc, base_url);
        var has_query = std.mem.indexOfScalar(u8, base_url, '?') != null;
        const encoded_query_0 = try core.url.percentEncode(alloc, status.toWire());
        defer alloc.free(encoded_query_0);
        try url_buf.print(alloc, "{s}status={s}", .{ if (has_query) "&" else "?", encoded_query_0 });
        has_query = true;
        const encoded_query_1 = try core.url.percentEncode(alloc, self.api_version);
        defer alloc.free(encoded_query_1);
        try url_buf.print(alloc, "{s}api-version={s}", .{ if (has_query) "&" else "?", encoded_query_1 });
        ...
        if (!responseStatusExpected(resp.status_code, &.{200})) {
            core.pager.logHttpError("Streams.updateStatus", resp.status_code, resp.body);
            return error.AzureRequestFailed;
        }
        return try serde.json.fromSlice(models.AuditStream, alloc, resp.body);
    }
